...
std::vector<std::tuple<char,char>> chessboard::get_movelist_at_position(const Colour& turn,const std::tuple<char, char>& start)
{	
	int x_pos=std::get<0>(start);
	int y_pos=std::get<1>(start);
	if(squares[x_pos][y_pos] != nullptr && squares[x_pos][y_pos]->get_colour() == turn){
		//Sets the movelist for the selected piece and position
		squares[x_pos][y_pos]->set_move_list(squares,start);
		return squares[x_pos][y_pos]->get_move_list();
	}
	else{
		return{};
	}
...
std::vector<std::tuple<char,char>> chessboard::get_capture_movelist_at_position(const Colour& turn,const std::tuple<char, char>& start)
{	
	int x_pos=std::get<0>(start);
	int y_pos=std::get<1>(start);
	if(squares[x_pos][y_pos] != nullptr && squares[x_pos][y_pos]->get_colour() == turn){
		//Sets the movelist for the selected piece and position
		squares[x_pos][y_pos]->set_move_list(squares,start);
		return squares[x_pos][y_pos]->get_capture_move_list();
	}
	else{
		return{};
	}
...
void chessboard::add_to_is_promotion_list(bool is_promotion)
{
	was_move_promotion.push_back(is_promotion);
}
...
{	
	...
	if(squares[x_start][y_start] != nullptr && squares[x_start][y_start]->get_colour() == turn){
		...
		//Checks that the inputted move is in the selected pieces move list
		if (std::find(all_moves.begin(), all_moves.end(), fin) != all_moves.end()){
			...
			if(squares[x_start][y_start]!=nullptr){
				...
				was_move_capture.push_back(true);
			}
			else{
				//If move wasn't a capture sets was last move capture to false
				was_move_capture.push_back(false);
			}
...
void chessboard::illegal_undo_move(const std::tuple<char, char>& start, const std::tuple<char, char>& fin)
{
	...
	if(was_move_promotion.back()==true){
	...
	//Swaps the unique ptrs to piece at the start and final position
	std::swap(squares[x_start][y_start],squares[x_fin][y_fin]);
	...
	if(was_move_capture.back()==true){
		squares[x_start][y_start]=std::move(graveyard.back());
		graveyard.pop_back();
	}

	was_move_capture.pop_back();
	was_move_promotion.pop_back(); 
			...
			for (int y{0}; y < 8; y++){
				...
				if(squares[x][y]!=nullptr && squares[x][y]->get_colour()==turn){
					...
					for (int i=0; i < possible_moves.size(); i++) {
						...
						//check for promotion as usual
						this->add_to_is_promotion_list(false);
...
{	
	...
	for (int x{0}; x < 8; x++){
		...
		if(squares[x][y_pawn_pos] != nullptr){
			...
			if (squares[x][y_pawn_pos]->get_piece_type() == Piece_Type::pawn){
				...
				was_move_promotion.push_back(true);
	...
	was_move_promotion.push_back(false);
	return false;
